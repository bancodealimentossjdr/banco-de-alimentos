// fix-pages.js - Atualiza a página de colheita solidária
const fs = require('fs');
const path = require('path');

const colheitaPage = `'use client'
...
export default function ColheitaSolidariaPage() {
  ...
  const getTotalQty = (items: HarvestItem[]) => items.reduce((sum, item) => sum + item.quantity, 0)
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            <div>
              <Link href="/" className="text-blue-500 hover:underline text-sm">← Voltar ao Painel</Link>
              <h1 className="text-3xl font-bold text-gray-900 mt-1">🌿 Colheita Solidária</h1>
            </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select value={form.indemnityValue}
                  onChange={e => setForm({ ...form, indemnityValue: parseFloat(e.target.value) })}
                  className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-amber-50">
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500" rows={1} />
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              {/* Cabeçalho dos itens */}
              <div className="hidden md:flex gap-3 items-end mb-1 px-1">
              ...
              <div className="space-y-3">
                {form.items.map((item, index) => (
                  <div key={index} className="flex gap-3 items-center">
                    ...
                    <div className="w-32">
                      <input type="number" step="0.1" min="0" value={item.quantity || ''}
                    ...
                    <div className="flex items-center gap-2">
                      <input type="checkbox" checked={item.weighed}
                        onChange={e => updateItem(index, 'weighed', e.target.checked)}
                        className="rounded border-gray-300 text-green-500 focus:ring-green-500" />
                      <label className="text-xs text-gray-500">Pesado</label>
                    </div>
            ...
            <div className="flex gap-3 mt-6">
              <button type="submit" className="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-lg font-medium transition">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {harvests.map(harvest => {
              ...
              return (
                <div key={harvest.id} className="bg-white rounded-xl shadow-sm border p-6">
                  <div className="flex flex-wrap justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        ...
                        {harvest.employee && (' • ' + harvest.employee.name)}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => startEdit(harvest)} className="text-blue-500 hover:text-blue-700 text-sm font-medium">Editar</button>
                      <button onClick={() => handleDelete(harvest.id)} className="text-red-500 hover:text-red-700 text-sm font-medium">Excluir</button>
                    </div>
                  </div>

                  {/* Itens com peso e indenização */}
                  <div className="mb-3">
                  ...
                  {/* Totais */}
                  <div className="flex flex-wrap gap-4 items-center pt-3 border-t border-gray-100">
                    <div className="flex items-center gap-2">
...
const colheitaDir = path.join('src', 'app', 'colheita-solidaria');
fs.writeFileSync(path.join(colheitaDir, 'page.tsx'), colheitaPage, 'utf8');
console.log('✅ src/app/colheita-solidaria/page.tsx atualizado!');
console.log('');
console.log('🎉 Colheita Solidária atualizada com indenização embutida no cadastro!');