  ...
  try {
    const reserva = await prisma.$transaction(async (tx) => {
      const alvo = await tx.reservaIngresso.findUnique({
        where: { id: reservaId },
        select: { id: true, cpf: true, retirado: true, retiradoEm: true },
      });
      ...
      // 🔒 Teto 3 por CPF no evento
      const jaRetirados = await tx.reservaIngresso.count({
        where: { cpf: alvo.cpf, retirado: true },
      });
      if (jaRetirados >= TETO_POR_CPF) {
        throw { status: 422, msg: `Limite de ${TETO_POR_CPF} ingressos por CPF atingido` };
      }

      // 🔒 Atômico: só marca se ainda estiver false (vence race entre tablets)
      ...
      if (upd.count === 0)
        throw { status: 409, msg: "Ingresso já retirado" };

      return tx.reservaIngresso.findUnique({
        where: { id: reservaId },
        select: { id: true, protocolo: true, nome: true, retirado: true, retiradoEm: true },
      });
    });
