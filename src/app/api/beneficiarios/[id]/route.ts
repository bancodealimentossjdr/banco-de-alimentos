...
export async function GET() {
  ...
  try {
    ...
    // 🔐 Aplica máscara se for visualizador
    const session = await auth()
    const beneficiariesSeguros = maskBeneficiarioList(beneficiaries, session?.user?.role)

    return NextResponse.json(beneficiariesSeguros)
  } catch (error) {
